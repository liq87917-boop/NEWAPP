...
GPT in the Codex desktop app is the only architecture decision and final acceptance authority for NEWAPP automation. The local controller must not call an OpenAI API or require an OpenAI API key.
...
11. Read pending requests from `.ai/brain/requests/` and record explicit decisions through the controller commands. Never let the DeepSeek/Cline executor write its own brain decision.
