...
def build(task: dict[str, Any], run_id: str, executor: dict[str, Any], validation: dict[str, Any], guard: dict[str, Any], browser: dict[str, Any]) -> dict[str, Any]:
    ...
    passed = executor.get("exit_code") == 0 and validation.get("status") == "passed" and guard.get("status") == "passed" and browser.get("status") in {"passed", "not_required"} and (not required or bool(screenshots))
    manifest = {
        "task_id": task["id"],
        "run_id": run_id,
        "created_at": utc_now(),
        ...
        "screenshots": {"required": required, "count": len(screenshots)},
        "artifacts": artifacts,
